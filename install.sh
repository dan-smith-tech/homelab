...
lvcreate -L 16G vg_system -n lv_swap
...
# install packages
pacman -S --noconfirm base base-devel efibootmgr egl-wayland git grub kitty linux linux-firmware linux-headers lvm2 networkmanager neovim nvidia-container-toolkit nvidia-open nvidia-utils opencl-nvidia openssh plasma-desktop qemu sudo virt-install
